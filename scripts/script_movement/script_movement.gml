...
if argument1 {
if free=false {vsp=-6}
}else{if vsp < 0 vsp+=0.4}

//Changes sprite
if spr_dir= 1 {if free=true {sprite_index=spr_jump_right}else{sprite_index=spr_walk_right}}
if spr_dir=-1 {if free=true {sprite_index=spr_jump_left}else{sprite_index=spr_walk_left}}
if free=false and hsp=0 and !argument0 and !argument2 {sprite_index=spr_stand}
...
if !place_meeting(x+sign(hsp),y,par_block) and !place_meeting(x+sign(hsp),y,par_unit) {trinfree=true}else{trinfree=false}
